...
std::vector<std::string> getOutputNames(const cv::dnn::Net &net)
{
    ...
    /*
    std::vector<cv::String> layersNames = net.getUnconnectedOutLayersNames();
    names.resize(layersNames.size());
    for (size_t i = 0; i < layersNames.size(); ++i)
    {
        names[i] = layersNames[i];
    }
    return names;
    */
...
int main()
{
    // molecular camera - stereo vision -- geometry
}
...
{
    // HAAR CASCADE DETECTION  -- face detection
    cv::Mat frame_gray;
...
void harris()
{
    ...
    output = cv::Mat::zeros(gray.size(), CV_32FC1);

    cv::cornerHarris(gray, output, 3, 3, 0.04);
    cv::normalize(output, output_norm, 0, 255, cv::NORM_MINMAX, CV_32FC1, cv::Mat());
    cv::convertScaleAbs(output_norm, output_norm_scaled); // convert to CV_8U
    ...
    for (int j = 0; j < output_norm.rows; j++)
    {
        for (int i = 0; i < output_norm.cols; i++)
        {
            ...
            {
                cv::circle(img, cv::Point(i, j), 4, cv::Scalar(0, 255, 0), 1, 4, 0);
            }
        }
    }
...
void shiTomasi()
{
    ...
    cv::goodFeaturesToTrack(gray, corners, 20, 0.01, 10, cv::Mat(), 3, false, 0.04);

    for (size_t i = 0; i < corners.size(); i++)
    {
        cv::circle(img, corners[i], 5, cv::Scalar(0, 255, 0), 2, 8, 0);
    }

    cv::imshow("SHI TOMASI CORNER DETECTION", img);
    cv::waitKey(0);
}
